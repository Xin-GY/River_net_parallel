...
"""
    ...
    def InBound_Fix_level_V2(self, level,
        ...
        self.S[0] = Ab
        self.Q[0] = Qb
        self.water_level[0] = level
        if hasattr(self, 'water_surface_width'):
            self.water_surface_width[0] = Tb
        if hasattr(self, 'water_depth') and hasattr(self, 'buttom_height'):
            self.water_depth[0] = max(level - self.buttom_height[0], 0.0)
    ...
    def OutBound_Fix_level_V2(self, level,
        ...
        self.S[-1] = Ab
        self.Q[-1] = Qb
        self.water_level[-1] = level
        if hasattr(self, 'water_surface_width'):
            self.water_surface_width[-1] = Tb
        if hasattr(self, 'water_depth') and hasattr(self, 'buttom_height'):
            self.water_depth[-1] = max(level - self.buttom_height[-1], 0.0)
    ...
    def InBound_Fix_level_V3(self, level,
                             Fr_max=0.85, head_gain_factor=0.65,
                             relax_Q=0.4, cap_du_factor=0.8, cap_dQ_factor=0.7,
                             use_stabilizers=True):
        """
        上游（左端）固定水位边界（FullSWOF-1D：Imposed stage + modified MOC）
        - 主体计算：保持 J- = u - 2c（离域特征）常值，先得 u_b，再得 Q_b = A_b*u_b
        - 一般断面声速：c = sqrt(g*A/T)
        ...
        if Ab <= 0.0:
            ...
            if hasattr(self, 'water_surface_width'): self.water_surface_width[0] = 0.0
            if hasattr(self, 'water_depth') and hasattr(self, 'buttom_height'):
                self.water_depth[0] = max(level - self.buttom_height[0], 0.0)
            if getattr(self, 'Implic_flag', False):
                self.V0[0] = self.S[0] - self.S_old[0]
    ...
    def OutBound_Fix_level_V3(self, level,
        ...
        if Ab <= 0.0:
            ...
            if hasattr(self, 'water_surface_width'): self.water_surface_width[-1] = 0.0
            if hasattr(self, 'water_depth') and hasattr(self, 'buttom_height'):
                self.water_depth[-1] = max(level - self.buttom_height[-1], 0.0)
            if getattr(self, 'Implic_flag', False):
                self.V1[0] = self.S[-1] - self.S_old[-1]
