...
# ===== 并行配置（直接改这里） =====
PARALLEL_MODE = 'process'   # 固定使用多进程
PARALLEL_WORKERS = 4        # 直接改这个数字控制进程数量
...
def build_net(export_graph=False):
    ...
    run_model_data['parallel_mode'] = 'process'
...
def main():
    ...
    parser.add_argument('--yield-step', type=float, default=30, help='Evolve 回报间隔(秒)')
    ...
    parser.add_argument('--print-progress', action=argparse.BooleanOptionalAction, default=True, help='打印演进信息')
    ...
    print(
        f"启动计算: mode=process, workers={PARALLEL_WORKERS}, yield_step={args.yield_step}, print_progress={args.print_progress}",
        flush=True
    )
    wall_elapsed, evolve_elapsed = run_once(
    ...
    print(
        f"运行完成: mode=process, workers={PARALLEL_WORKERS}, "
        f"wall={wall_elapsed:.3f}s, evolve={evolve_elapsed:.3f}s",
        flush=True
    )
