...
# --------- Monkey patches for Rivernet (ThreadPool + Coloring) ---------
def _patch_rivernet_parallel():
    if getattr(Rivernet, "_parallel_patched", False):
        return

    # add initializer to create a persistent thread pool
    _orig_init = Rivernet.__init__
    def _init(self, Topology, model_data, verbos=True, parallel_workers=None):
        ...
        def _set_parallel_workers(new_workers):
    ...
    def _parallel_map(self, method_name):
        rivers = _all_rivers(self)
        if not getattr(self, "_parallel_enabled", False):
            for r in rivers:
                getattr(r, method_name)()
            return

        futures = [self._pool.submit(getattr(r, method_name)) for r in rivers]
        for f in futures:
            f.result()

    # replace seven heavy *_net methods
    ...
    # graph-coloring internal nodes; batch-parallel apply+iterate
    ...
    def _Update_internal_boundary_conditions(self):
        ...
        # conflict if two internal nodes share a direct river (are adjacent in G)
        for u, v in self.G.edges():
        ...
        # solving function for a single node (contains apply + iterate)
        def _solve_node(node):
            iter_time = 1
            dz = 999.0
            pure_Q = 999.0
            node_level = self.Caculate_node_average_level_at_real_cell(node)
            if node_level < 0: node_level = 0.0
        ...
        # process batches sequentially; inside each batch run in parallel
        for c, nodes in sorted(batches.items()):
            if not getattr(self, "_parallel_enabled", False):
                for n in nodes:
                    _solve_node(n)
            else:
                futures = [self._pool.submit(_solve_node, n) for n in nodes]
                for f in futures:
                    f.result()
