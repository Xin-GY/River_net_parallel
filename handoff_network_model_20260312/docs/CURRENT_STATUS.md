...
目录：`result/exp_stage_target_cache_process_py311_40h/`
...
- Islam 40h full-case 模型内部自报时间：`147.45 s`
- Islam 40h full-case `/usr/bin/time` wall：`154.72 s`
- 当前验收优先看模型内部自报演进时间，不计初始化时间

解释：
- 本轮最新性能提升来自“固定水位边界目标侧 `level -> (A, T, general_chi, depth)` 预构建缓存”
- 该缓存只作用于等价几何/特征量重建，不改变节点残差和迭代更新公式
- 默认只保留 `river11_interpolated_output.nc`
- 若需要历史双文件行为，可设 `ISLAM_OUTPUT_WRITE_MODE=legacy_dual`
- 与上一接受版 `result/exp_saveinterval_yield_process_py311_40h` 严格 compare 为 `allclose = true`
...
2. 当前最快 CPU 路径已经把 40h full-case 的模型内部自报时间压到 `147.45 s`。
3. 新输出口径下，`internal_node_history.csv` 与 `river11_interpolated_output.nc` 均逐点一致。
4. `interpolated_output.nc` 现在是唯一默认产物，因此下游脚本应优先依赖它。
5. 新增的 stage-target 预构建缓存已经命中默认内部汊点 fast path，且结果逐点一致。
6. 若继续提速，下一步应回到单河道数值热点或进程通信，而不是继续压输出构建。
