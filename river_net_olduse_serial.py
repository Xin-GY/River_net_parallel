...
class Rivernet:
    def __init__(self, topology, model_data, verbos: bool = True):
        ...
        self.alpha = float(self.model_data.get("junction_alpha", 1.2))
    ...
    def Caculate_node_Ac_at_ghost_cell(self, node):
        ...
        for _, _, data in self.G.in_edges(node, data=True):
            river = data["river"]
            area = max(float(river.S[-1]), eps_area)
            width = max(float(river.cross_section_table.get_width_by_area(river.cell_sections[-1], area)), eps_width)
            flow = float(river.Q[-1])
            ac += math.sqrt(self.g * area * width) - flow * width / area

        for _, _, data in self.G.out_edges(node, data=True):
            river = data["river"]
            area = max(float(river.S[0]), eps_area)
            width = max(float(river.cross_section_table.get_width_by_area(river.cell_sections[0], area)), eps_width)
    ...
    def Apply_node_target_level_V4(self, node_name: str, level: float, Kj: float = 0.0, regime: str = "sub") -> None:
        ...
        for _, _, data in self.G.in_edges(node_name, data=True):
            river = data["river"]
            area = max(float(river.S[-2]), eps_area)
            flow = float(river.Q[-2])
            velocity = abs(flow) / area
            dH = Kj * velocity * velocity / (2.0 * self.g)
            level_eff = level + (dH if flow > 0.0 else -dH)

            if regime == "super_in":
                river.S[-1] = river.S[-2]
                river.Q[-1] = river.Q[-2]
                river.water_level[-1] = level
            else:
                self._call_stage_out(river, level_eff)

        for _, _, data in self.G.out_edges(node_name, data=True):
            river = data["river"]
            area = max(float(river.S[1]), eps_area)
            flow = float(river.Q[1])
            velocity = abs(flow) / area
            ...
            if regime == "super_in":
                river.S[0] = river.S[1]
                river.Q[0] = river.Q[1]
                river.water_level[0] = level
            else:
                self._call_stage_in(river, level_eff)
    ...
    def Update_internal_boundary_conditions(self):
        for node in self.internal_nodes:
            ...
            while True:
                ...
                self.Apply_node_target_level_V4(node, node_level)
                ac = self.Caculate_node_Ac_at_ghost_cell(node)
                pure_Q = self.Get_node_clear_flow_at_ghost_cell_net(node)
                dz = 0.0 if abs(ac) <= 1.0e-12 else pure_Q / ac

                iter_time += 1
                node_level += self.relax * dz

                converged = (
                    (abs(dz) < 1.0e-4 or abs(dz) / max(abs(node_level), 1.0e-3) < 1.0e-5)
                    and abs(pure_Q) < self.JPWSPC_Q_limit
                )
                if converged:
                    self.Apply_node_target_level_V4(node, node_level)
                    break
    ...
    def export_png(
        ...
        nx.draw_networkx_nodes(self.G, pos, nodelist=self.internal_nodes, node_color="#4C78A8", node_size=700, ax=ax, label="internal")
        nx.draw_networkx_nodes(self.G, pos, nodelist=self.external_in_nodes, node_color="#F58518", node_size=600, ax=ax, label="external_in")
        nx.draw_networkx_nodes(self.G, pos, nodelist=self.external_out_nodes, node_color="#54A24B", node_size=600, ax=ax, label="external_out")
        nx.draw_networkx_edges(self.G, pos, ax=ax, arrows=True, arrowstyle="-|>", arrowsize=18, width=1.8, alpha=0.95)

        if show_node_labels:
            nx.draw_networkx_labels(self.G, pos, ax=ax, font_size=10)
        if show_edge_labels:
            edge_labels = {(u, v): data.get("name", f"{u}->{v}") for u, v, data in self.G.edges(data=True)}
            nx.draw_networkx_edge_labels(self.G, pos, edge_labels=edge_labels, ax=ax, font_size=9, rotate=False)
